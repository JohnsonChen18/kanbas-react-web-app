        ...
        <div id="wd-working-with-arrays">
            ...
            <div className="d-flex flex-row align-items-center">
                <input value={todo.id} className="form-control w-25 float-start"
                       onChange={(e) => setTodo({...todo, id: e.target.value})}/>
                <input className="form-check-input ms-2" id="wd-todo-completed-true" type="checkbox"
                       checked={todo.completed} onChange={(e) =>
                    setTodo({...todo, completed: true})}/>
                <label className="form-check-label me-2" htmlFor="wd-todo-completed-true">
                    True
                </label>
                <input className="form-check-input " id="wd-todo-completed-false" type="checkbox"
                       checked={!todo.completed} onChange={(e) =>
                    setTodo({...todo, completed: false})}/>
                <label className="form-check-label me-2" htmlFor="wd-todo-completed-false">
                    False
                </label>
                <a href={`${API}/${todo.id}/completed/${todo.completed}`} className="btn btn-primary">
                    Update Todo Completed State</a>
            </div>
            <br/>
            ...
            <input id="wd-todo-id" value={todo.id} className="form-control w-25 float-start me-2"
                   onChange={(e) => setTodo({...todo, id: e.target.value})}/>
            <input id="wd-todo-description" value={todo.description} className="form-control w-25 float-start"
